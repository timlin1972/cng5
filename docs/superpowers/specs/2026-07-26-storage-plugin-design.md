...
- 不做存取控制/認證：`/api/storage/...` 這幾個端點沿用整個專案既有的做法（`files`/
  `music`/`notepad` 的端點也都沒有），只要連得到這台機器的 9759 port 就能列出/上傳/
  下載/刪除 `storage/` 底下的內容。這是整個專案層級的既有取捨，不是這個 plugin 特別
  放寬的，但因為這個功能本身就是「開放檔案系統存取」，值得在這裡明講一次。
